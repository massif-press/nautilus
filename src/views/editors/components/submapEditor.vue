<template>
  <v-tabs v-if="!noAdd || item.Submaps.length" v-model="tab" density="compact" height="30px">
    <v-tab v-for="(s, i) in item.Submaps" :value="i">{{ s.Name }}</v-tab>
    <v-tab v-if="!noAdd && item.isUserOwned" prepend-icon="mdi-plus" @click="addLOD">Add</v-tab>
  </v-tabs>
  <div v-if="!item.Submaps.length" class="text-center text-caption text-disabled my-2">
    <i>No submaps</i>
  </div>
    ...
    <v-window-item v-for="(s, i) in item.Submaps" :key="i">
      <v-card-text>
        <v-row>
          ...
          <v-col cols="auto" v-if="item.isUserOwned">
        ...
        <submap-selector
          ...
          :selected="item.Submaps[tab]"
          @select="setSubmapImg($event)" />
        ...
        <div v-if="item.isUserOwned">
          ...
          <div class="text-caption text-disabled mt-1">Labels for {{ s.Name }}</div>
          <v-table density="compact">
            <thead>
              <tr>
                <th>Label</th>
            ...
            <tbody>
              <tr
                v-for="item in item.Submaps[tab].Labels"
                class="text-center"
                :class="selected && item.ID === selected.ID ? 'bg-primary' : ''">
                <td>
                  <v-text-field
                ...
                <td>
                  <v-menu :close-on-content-click="false">
                    ...
                    <v-color-picker v-model="item.Color" hide-details @change="rerender" />
                  </v-menu>
                </td>
                <td>
                  <v-text-field
